...
mycon = pymysql.connect(host= 'localhost', user='root', password = 'password', database = 'snake_game')
...
def leaderboard_stat():                                                                                                 # function to get top 5 players
    lead = list(display_sql())                                                                                                # collects all data from mysql
    players = len(lead)
...
def about_func():                                                                                                       # function for when you click on about button
    ...
    message("Additional thank you to abhijit for music suggestions and our friends for beta testing.", (255, 255, 255), 150, 240, 25) # blits the message
    message("Moreover we have gratitude for our teacher for providing us this golden opporutunity", (255, 255, 255), 140, 270, 25)   # blits the message
...
def enter_sql(name, score, speed, size):                                                                                # function for entering new data in mysql
    ...
    statement_enter = "INSERT INTO scores(name, score, speed, size) values('{}','{}','{}','{}')".format(name, score, speed, size)                                                                                                                        # statement for updating existing value
    statement_update = "UPDATE scores set score = '{}', speed = '{}' ,size = '{}' WHERE name = '{}'".format(score, speed, size, name)
...
def end_screen(display_width, display_height, size, speed, size_of_display, image, score):                              # screen shown when the game ends
    ...
    lead = list(display_sql())                                                                                                # gets the data from sql
...
def enter_game(speed, snake_color, size_of_display, wallpaper, food_type, music_type, name):                                                                                                                  # function for the full game part
    ...
    def message(msg, color, width_location, height_location, size):                                                     # function for putting messages in window
        mesg = pygame.font.SysFont("Corbel", size).render(msg, True,color)                                                                                                                  # setting font and size and color of message
        ...
        while not game_over[0]:                                                                                         # loop for whole game
            while game_close[0]:                                                                                        # loop for each repeating of game
                ...
                if flag_3[0] == 1:
                    ...
                    image = pygame.transform.scale(image,(display_width, display_height - 30))                                                                                   # setting size of wallpaper
            ...
            message("Score = " + str(score) + "    Highscore = " + str(highscore), colors[1], 5, 5,size=20)                                                                                                                # putting the score and highscore on the top
            ...
            if (x == foodx or x == foodx + 10 or x == foodx + 20) and (y == foody or y == foody + 10 or y == foody + 20):                                                                      # condition to check if snake eats food
